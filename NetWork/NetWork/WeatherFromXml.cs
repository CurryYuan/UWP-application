...
namespace NetWork
{
    public class WeatherFromXml
    {
        public async static Task<Current> GetWeather(double lat, double lon)
        {
            var http = new HttpClient();
            string source = string.Format("http://api.openweathermap.org/data/2.5/weather?lat={0}&lon={1}&units=metric&mode=xml&appid=62c4e931a32545a6bbde5b248d89e929", lat, lon);
            var response = await http.GetAsync(source);
            var result = await response.Content.ReadAsStringAsync();
            var serializer = new XmlSerializer(typeof(Current));

            var ms = new MemoryStream(Encoding.UTF8.GetBytes(result));
            var data = (Current)serializer.Deserialize(ms);
            return data;
        }

        public async static Task<Current> GetWeather(string name)
        {
            var http = new HttpClient();
            string source = string.Format("http://api.openweathermap.org/data/2.5/weather?q={0}&units=metric&mode=xml&appid=62c4e931a32545a6bbde5b248d89e929", name);
            var response = await http.GetAsync(source);
            var result = await response.Content.ReadAsStringAsync();
            var serializer = new XmlSerializer(typeof(Current));
